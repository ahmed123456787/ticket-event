...
class EventTicketViewSet(ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == 'check_in':
            return TicketCheckInSerializer
        elif self.action == 'purchase':
            return TicketPaymentSerializer
    ...
    def check_in(self, request):
        ...
        if serializer.is_valid():
            # Get the existing ticket from validated_data
            ticket = serializer.validated_data['ticket']

            ticket = EventTicket.objects.get(ticket_code=ticket.ticket_code)
